...
export default function CalculatorDock({ dockedSide, geometry, onDock, onRestore }) {
  ...
  return (
    <button
      ...
      className={`no-print fixed left-0 top-0 z-[45] flex h-12 touch-none items-center justify-center gap-2 border border-slate-700/80 bg-slate-900/95 px-3 py-2 text-blue-500 dark:text-blue-300 shadow-xl shadow-black/30 backdrop-blur-xl hover:bg-slate-800 hover:border-slate-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-400 ${
        isDragging ? 'cursor-move select-none' : 'cursor-pointer transition-[background-color,transform] duration-200'
      } ${
