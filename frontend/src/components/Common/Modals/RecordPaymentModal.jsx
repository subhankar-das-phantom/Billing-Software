                ...
                <div className="p-4 sm:p-6 space-y-4 overflow-y-auto">
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-slate-400">
                      Search by customer name, phone number, address, or GSTIN to record a payment
                    </p>
                    <div className="relative mt-2">
                      ...
                      <input
                        ...
                        placeholder="Search customer name, phone, address, or GSTIN..."
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-3 min-w-0 flex-1">
                                <div className="w-9 h-9 rounded-lg bg-emerald-500/10 text-emerald-400 flex items-center justify-center font-bold text-sm shrink-0 group-hover:scale-105 transition-transform">
                                  {cust.customerName?.charAt(0)?.toUpperCase() || 'C'}
                                </div>
                                <div className="min-w-0 flex-1">
                                  <p className="text-sm font-medium text-slate-100 group-hover:text-emerald-300 transition-colors truncate">
                                    {cust.customerName}
                                  </p>
                                  <div className="flex flex-wrap items-center gap-x-2 text-xs text-slate-400">
                                    <span>{cust.phone || 'No phone'}</span>
                                    {cust.gstin && (
                                      <span className="font-mono text-[11px] bg-slate-800/80 px-1.5 py-0.5 rounded border border-slate-700/60 text-slate-300">
                                        {cust.gstin}
                                      </span>
                                    )}
                                  </div>
                  ...
                  {!initialCustomer && activeCustomer && (
                    <div className="flex items-center justify-between p-3 rounded-xl bg-slate-700/40 border border-slate-600/50">
                      <div className="flex items-center gap-2.5 min-w-0 flex-1">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-slate-100 truncate">{activeCustomer.customerName}</p>
                          <div className="flex flex-wrap items-center gap-x-2 text-xs text-slate-400">
                            <span>{activeCustomer.phone || 'No phone'}</span>
                            {activeCustomer.gstin && (
                              <span className="font-mono text-[10px] bg-slate-800/80 px-1.5 py-0.5 rounded border border-slate-700/60 text-slate-300">
                                {activeCustomer.gstin}
                              </span>
                            )}
                          </div>
                          {activeCustomer.address && (
                            <p className="text-xs text-slate-400 flex items-center gap-1 mt-0.5 truncate">
                              ...
                              <span className="truncate" title={activeCustomer.address}>{activeCustomer.address}</span>
                            </p>
                          )}
                        </div>
                      </div>
