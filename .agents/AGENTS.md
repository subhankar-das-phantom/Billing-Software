...
- **Zero-CLS Layout Stability & Anti-Lag**:
  ...
  - **Dropdown, Popover & Floating Panel Anti-Flicker Engineering**:
    - Never animate dropdown menus, popovers, or floating cards with spring physics (`type: 'spring'`) or center-origin scaling (`scale: 0.95`). Spring oscillations trigger subpixel anti-aliasing shimmering on 1px borders (`border-slate-700/80`), deep box-shadow rasterization thrashing (`shadow-2xl shadow-black/60`), and text blurring on Chromium.
    - Always animate floating panels using GPU-accelerated opacity & subtle Y-glide (`initial={{ opacity: 0, y: -6 }}`, `animate={{ opacity: 1, y: 0 }}`, `exit={{ opacity: 0, y: -6 }}`, `duration: 0.15s, ease: [0.16, 1, 0.3, 1]`) with explicit `style={{ transformOrigin: 'top right' }}` (or `'top left'`) and `will-change-[transform,opacity]`.
    - Trigger buttons must lock into active open styling when `isOpen`/`aria-expanded="true"` (`isOpen ? 'bg-slate-800 border-slate-600 text-slate-100 shadow-xs' : 'bg-slate-800/40 hover:bg-slate-800/80...'`) so moving the cursor across the gap into the menu never causes the button to lose hover and trigger color fade flashes.
    - Trigger button inner children (avatar, text, chevron) must carry `pointer-events-none` to guarantee stable click target resolution.
    - Synchronize indicator icon rotation durations (e.g. `duration-150`) to the dropdown panel's animation duration (`0.15s`).
...
- **Defensive Client Engineering & Race Condition Immunity**:
  - Eliminate all race conditions before coding. Abort out-of-order network responses (`AbortController` / TanStack Query), prevent double-trigger event races (`onMouseDown` vs `onClick`), lock rapid double-submits synchronously with `useRef`, and anchor asynchronous background updates (FIFO/batch allocation) to immutable unique IDs (`_rowId`) rather than transient array indexes. Memoize sorted lists with `useMemo`.
  - **Defensive Table Column Renderers & Method Safety**: Table and list renderers must never assume single-schema populated objects (e.g. `item.product.name`, `item.ratePerUnit`). Always provide nullish-coalescing fallbacks for flat structures (`item.product?.name ?? item.name`, `item.ratePerUnit ?? item.rate ?? 0`, `item.quantitySold ?? item.quantity ?? 0`). Guard all `.toFixed()`, `.slice()`, and `.charAt()` calls with nullish coalescing or optional chaining (e.g. `(Number(val) || 0).toFixed(2)`).
  - **Mock Adapter & Demo Data Parity**: Mock adapters must maintain 100% data isolation per entity ID (never return global arrays for specific customer/supplier endpoints). Mock handlers must synchronize both root `entity.*` and `summary.*` fields to eliminate split KPI discrepancies across pages. Mock relations must include valid relationship IDs (`invoiceId`, `invoice: { _id, invoiceNumber }`) to prevent broken `/undefined` navigation routes.
...
- **Step 6 — In-Chat Copy-Paste-Ready Release Notes**: When `master` is merged, tagged, and pushed, **output the complete, unabridged GitHub Release Notes directly inside a fenced markdown codeblock in the chat response** (never just link to an artifact file). Include the direct pre-filled GitHub release URL above the block. If multiple versions were merged, provide a dedicated link and codeblock for each version so the user can copy and paste with 1 click.
  - **Zero Local Filesystem Paths (Public Release Sanitation)**: Inside the GitHub release notes codeblock, **strictly use clean repository-relative paths** (`frontend/...`, `backend/...`) or code backticks (`` `Component.jsx` ``). **NEVER output local filesystem URLs (`file:///...`), Windows drive letters (`C:\`, `D:\`), or local PC paths**, as these leak private system directories and are broken on public GitHub Releases.
