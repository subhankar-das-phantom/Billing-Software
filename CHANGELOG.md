...
- **Side-by-Side 2-Column Hero Layout (`HeroSection.jsx`)** — Re-architected desktop layout into a balanced 2-column grid (`grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 xl:gap-14 items-center`):
  ...
  - **Right Column (`lg:col-span-7`)**: Embeds `ProductWindow` and **3 rich floating notification cards** (`hidden xl:flex`) stacked vertically on the right edge with subtle backdrop blur and deep borders:
    - *Payment Received*: Emerald checkmark icon, `₹2,912.00 (Gupta Medicos)`, `2 minutes ago`.
...
- **Showcase Screenshot Re-Capture with Zero Overflow (`dashboard.png`, `dashboard.webp`)** — Re-captured all 6 showcase screenshots at 1440×900 Retina 2x via Puppeteer following the 5-grid card overflow resolution in `DashboardKPIGrid.jsx`. Verified that in the newly generated public assets (`dashboard.png`, `dashboard.webp`, `dashboard-mobile.webp`, `dashboard-sm.webp`), the KPI cards render in 3 comfortable columns with plenty of breathing room, zero horizontal overflow, and flawless alignment.
