...
- **Side-by-Side 2-Column Hero Layout (`HeroSection.jsx`)** — Re-architected desktop layout into a balanced 2-column grid (`grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 xl:gap-14 items-center`):
  ...
  - **Right Column (`lg:col-span-7`)**: Embeds `ProductWindow` referencing newly versioned, cache-immune assets (`/landing/product/dashboard-v2.webp`) and **2 focused floating notification cards** (`hidden xl:flex`) stacked vertically on the right edge with subtle backdrop blur and deep borders:
    - *Monthly Revenue*: Blue bar chart icon, `₹3,90,546 (+93.4%)`, `vs. last month`.
    - *Catalog*: Neutral package icon, `{productsCount} SKUs • {batchesCount} Active Batches`, `Across all categories`.
