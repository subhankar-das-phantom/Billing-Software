...
- **Global Excel Engine IST Alignment (`excel.ts`)** — Enhanced `buildWorkbook` to adjust date and datetime column values by the Indian Standard Time delta (`+5.5 hours`) relative to the host server offset. Globally resolves Excel timestamp drift across Inventory Movements, Collections, and Purchases in a single architectural pass.
