...
- **Record Payment Modal Zero-CLS Anti-Flicker Architecture (`RecordPaymentModal.jsx`, `InvoiceViewPage.jsx`)** — Eliminated visual flicker, border shimmering, and layout jumping when opening the Record Payment Modal from the Invoice View page.
  - **Synchronous Frame-0 State Pre-Seeding**: Resolved the 1-frame asynchronous `useEffect` lag where `formData.selectionId` was initialized empty (`''`) on mount, rendering a collapsed modal on Frame 0 before asynchronously popping in invoice details on Frame 1. Added synchronous state seeding on mount and during render transitions (`isOpen && !prevIsOpen`), ensuring `selectedItem` is populated before DOM paint.
  - **Hardware-Accelerated Anti-Flicker Easing**: Eliminated spring physics (`stiffness: 300, damping: 30`), `scale: 0.95`, and card `backdrop-blur-xl` which caused 60 frames of subpixel anti-aliasing shimmering on 1px borders and GPU compositing thrashing. Standardized to `scale: 0.98, y: 8` with snappy GPU-accelerated cubic-bezier (`duration: 0.15s, ease: [0.16, 1, 0.3, 1]`) and `will-change-[transform,opacity]`.
  - **Zero-CLS Static Containers**: Replaced layout-shifting `<motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }}>` on the selected invoice card and FIFO preview with static `div` containers, eliminating mid-animation ~140px vertical reflow jumps.
  - **Parent Prop Stability**: Wrapped `totalCreditNoteAmount`, `netDue`, `invoiceForPayment`, and `invoiceForPaymentList` in `useMemo` in `InvoiceViewPage.jsx` to eliminate redundant modal re-evaluations from unstable object and array references.
- **Invoice View Action Bar Redesign & Control Deduplication (`InvoiceViewPage.jsx`)** — Redesigned the fragmented 11-button action bar into an Enterprise 2-Tier Header Card. Tier 1 houses document identity (`INV-2026-001`), semantic status badge, live payment status pill (`Due: ₹...` / `Fully Paid`), back link, and prominent primary CTAs (`Record Payment`, `Print Invoice`). Tier 2 houses the print & export utility strip: a connected segmented Copy Mode capsule switch (`[ 1x Single | 2x Double ]`), a zero-CLS floating Columns popover with dynamic count badge (`8/12`), a `Mark Printed` action (when status is `Created`), PDF Download and Share buttons with copy tooltip, and an anchored `Manage (•••)` dropdown menu housing secondary actions (`Edit Invoice`, `Create Return`, and red `Cancel Invoice`). Replaced the layout-shifting inline `height: auto` column box with an anchored zero-CLS floating popover (`0.15s`, `transformOrigin: 'top left'`, locked active trigger styling). Pruned the redundant standalone Copy Mode card below the payment summary and the duplicate Record Payment button in the summary card.
- **Sales Return Inventory Ledger Audit Trail & Attribution Parity (`creditNoteController.js`, `inventoryService.ts`, `stockMovementService.ts`, `ProductDetailsPage.jsx`, `backfillStockMovements.ts`)** — Resolved the critical issue where creating a Sales Return / Credit Note failed to appear in the Inventory Ledger (`/inventory/ledger`) and rendered broken `- → -` stock levels with `By: System` in Product Details (`/products/:id`).
  - **Root Cause & Fix**: Non-batch credit note item processing in `creditNoteController.js` previously only updated `Product.currentStockQty` and never invoked `recordStockMovement`. Refactored the controller to pre-generate `creditNoteNumber`, query current product stock in-session to compute exact `previousQty` and `newQty: previousQty + returnItem.quantityReturned`, pass `adjustedBy: getAttribution(req)`, and call `inventoryService.recordStockMovement` with `type: 'SALE_RETURN'`, `referenceType: 'CreditNote'`, and `referenceId: creditNoteNumber`.
  - **Batch Return Parity**: Extended `inventoryService.restoreBatchAllocations` to support `type: 'sales_return'` and optional `referenceType`, `referenceId`, and `createdBy` attribution options.
  - **Admin Attribution Resolution**: Fixed the `$project` stage in `stockMovementService.ts` to fall back to `$admin.firmName` if `$admin.name` is null, preventing Admin operations from incorrectly displaying as `System`.
  - **Defensive Historical UI Guard**: Added defensive fallback in `ProductDetailsPage.jsx` to render `+X returned` whenever historical records lack `previousQty`, eliminating `- → -`.
  - **Historical Credit Note Reconciliation**: Enhanced `backend/scripts/backfillStockMovements.ts` with Phase 2 for Credit Notes, scanning all historical credit notes and safely, idempotently inserting missing `SALE_RETURN` movements into `StockMovement`.
- **Skills & Engineering Contracts Hardening (`AGENTS.md`, `frontend-architect/SKILL.md`, `implementation-planner/SKILL.md`)** — Formally codified architectural mandates to permanently eliminate dropdown flicker, dual-shape column rendering crashes, and mock adapter parity drift across all future agent interactions. Added explicit rules forbidding spring physics and center-origin scaling on floating elements, requiring GPU-accelerated opacity/Y-glide (`duration: 0.15s, ease: [0.16, 1, 0.3, 1]`) with `transformOrigin` and `will-change-[transform,opacity]`, enforcing locked active styling on trigger buttons when open (`aria-expanded="true"`), requiring `pointer-events-none` on button children, mandating nullish-coalescing fallbacks for nested vs flat data across all table column renderers, and requiring 100% ID isolation and dual-source (`entity.*` vs `summary.*`) synchronization in mock adapters.
- **User Profile Dropdown Flicker Elimination & Zero-CLS GPU Acceleration (`Header.jsx`)** — Resolved visual flicker and jitter when toggling or hovering the Header user profile menu. Eliminated spring physics (`stiffness: 450, damping: 30`) and center-origin scaling (`scale: 0.95`) which triggered subpixel anti-aliasing shimmering on the 1px border (`border-slate-700/80`), deep box-shadow rasterization thrashing (`shadow-2xl shadow-black/60`), and text blurring on Chromium. Replaced with GPU-accelerated opacity and subtle Y-glide (`initial={{ opacity: 0, y: -6 }}`, `duration: 0.15s, ease: [0.16, 1, 0.3, 1]`) anchored with `transformOrigin: 'top right'` and `will-change-[transform,opacity]`. Prevented button hover-loss flashing when navigating the pointer across the 8px `mt-2` gap by explicitly locking the active button styling when `profileOpen` is true (`profileOpen ? 'bg-slate-800 border-slate-600 text-slate-100 shadow-xs' : '...'`), isolated inner pointer events (`pointer-events-none` on avatar, typography, and chevron), and synchronized chevron rotation to 150ms.
- **Invoice View Crash Fix — Defensive Column Renderers (`InvoiceViewPage.jsx`)** — Resolved `Uncaught TypeError: Cannot read properties of undefined (reading 'toFixed')` at line 161 caused by demo invoice items carrying flat-field shape (`rate`, `quantity`, `name`, `gstRate`, `batchNumber`, `expiryDate`) while all column renderers assumed populated `product.*` objects and `ratePerUnit`/`quantitySold`. Applied `??` nullish-coalescing fallbacks across all eight column renderers: Qty (`quantitySold ?? quantity`), Product Name (`product?.productName ?? productName ?? name`), HSN (`product?.hsnCode ?? hsnCode`), Batch (`product?.batchNo ?? batchNumber ?? batchNo`), Expiry (`product?.expiryDate ?? expiryDate`), MRP (`product?.newMRP ?? mrp ?? newMRP`), Rate (`ratePerUnit ?? rate`), Net, GST% (`product?.gstPercentage ?? gstPercentage ?? gstRate`), and Amount (`totalAmount ?? qty × rate`). Both real backend items and demo flat-shape items now render safely.
- **Payments Tab `/invoices/undefined` Link Fix (`demoData.js`)** — Resolved broken `/invoices/undefined` navigation in the Customer Details Payments tab. `DEMO_COLLECTIONS` entries had `invoiceNumber` but lacked `invoiceId` and `invoice._id`, causing `payment.invoice?._id || payment.invoiceId` to resolve to `undefined`. Added `invoiceId` and `invoice: { _id, invoiceNumber }` to all three collection records (col_001 → `inv_005`, col_002 → `inv_006`, col_003 → `inv_004`), restoring correct deep-link navigation to the individual invoice view.
- **Per-Customer Data Isolation — Invoices, Payments & Ledger (`demoAdapter.js`)** — Resolved inflated badge counts and cross-customer data bleed in CustomerDetailsPage. `summary.invoiceCount` was reading the aggregate `totalInvoicesCount` from `DEMO_CUSTOMERS` (e.g. 19 for Shreeji Pharma) instead of the actual number of `DEMO_INVOICES` records for that customer. `summary.paymentCount` was hardcoded to `3`. `GET /payments/customer/:id` returned all `DEMO_COLLECTIONS` regardless of `customerId`. `GET /customers/:id/ledger` always emitted the same two hardcoded entries. All three handlers now derive data dynamically: `invoiceCount` and `paymentCount` from filtered `DEMO_INVOICES`/`DEMO_COLLECTIONS`, the payments endpoint filters by `customerId`, and the ledger is built per-customer by merging that customer's invoices (as debit rows) and payments (as credit rows), sorted chronologically with a correct running balance computed to 2 decimal places.
- **Customer KPI Cards — Live Data Derivation & Dual-Source Fix (`demoAdapter.js`)** — Resolved `₹2,65,000 Total Purchases` and `19 Invoices` KPI cards still displaying stale values after the isolation fix. Root cause: `CustomerDetailsPage` renders two KPI cards (`Total Purchases`, `Invoices count`) directly from `customer.totalPurchases` and `customer.invoiceCount`, bypassing `summary.*` entirely. The adapter was correcting `summary` but returning `found` (the raw `DEMO_CUSTOMERS` record with historical aggregates) as the `customer` object. Fix: `GET /customers/:id` now constructs an `enrichedCustomer` by spreading the live-computed values (`totalPurchases`, `totalPurchasesAmount`, `invoiceCount`, `totalInvoicesCount`, `outstandingBalance`, `totalOutstanding`) over the raw record before returning, ensuring 100% parity across both `customer.*` and `summary.*` bindings. `totalPurchases` is the sum of `grandTotal` across the customer's actual demo invoices; `outstandingBalance` is the sum of `remainingAmount`; `invoiceCount` is the filtered invoice array length. All per-customer ground truth: Patel Medical ₹38,400 / 1 inv, Shreeji Pharma ₹62,500 / 1 inv, Krishna Drug House ₹30,400 / 1 inv (₹15,200 outstanding), Maruti Medicos ₹24,200 / 1 inv (₹0 outstanding), Apollo Pharmacy ₹32,120 / 1 inv, Apex Healthline ₹21,000 / 1 inv (₹0 outstanding).
- **Activity Log Crash Fix & Defensive Normalization (`ActivityLogPage.jsx`, `demoData.js`, `demoAdapter.js`)** — Resolved `Uncaught TypeError: Cannot read properties of undefined (reading 'slice')` in `SessionCard`. Implemented defensive activity extraction supporting both structured object and flat array activities, guarded all `.slice(...)` calls and `.length` lookups against undefined or missing activity categories, and protected `VirtualizedList` key extraction. Aligned `DEMO_EMPLOYEE_ANALYTICS.activityLog.log` with the backend `ActivityLogEntry` schema, providing top-level employee records and itemized invoices (`INV-2026-001`, `INV-2026-004`), UPI/NEFT payments, and catalog movements.
- **Customer Khata & Invoices Parity (`demoData.js`, `demoAdapter.js`)** — Restored complete parity for Customer Details (`/customers/:id`, e.g. Patel Medical & Surgicals). Added `totalPurchases` and `invoiceCount` keys to `DEMO_CUSTOMERS`, added `summary` calculation to `GET /customers/:id`, and added mock endpoints for `GET /invoices/customer/:id`, `GET /payments/customer/:id`, and `GET /customers/:id/ledger`. Eliminates zero counter displays, restores the invoice history table, and renders closing ledger balances down to the exact paisa.
- **Sales & Collections Trend Natural Randomization (`demoData.js`)** — Replaced flat, locked 88% daily collections with authentic business variance: independent weekday sales spikes (₹18,000–₹46,200), weekend dips (₹6,400–₹16,800), and lagged payment clearing waves (₹5,200–₹44,200). Normalized monthly dataset to numerical `month: 1..12` indexes, fixing Recharts month label resolution.
- **Product Stock Movement History (`demoData.js`, `demoAdapter.js`)** — Added `GET /products/:id/stock-history` mock handler intercepting before the generic product route. Returns itemized stock movement history records with change quantities, previous/new stock levels, reference documents, and operator badges.
- **Inventory Ledger Audit Trail & Valuations (`demoData.js`, `demoAdapter.js`)** — Defined canonical `DEMO_STOCK_MOVEMENTS` array and wired `GET /stock-movements` with multi-field filtering (`type`, `productId`, `batchId`, `dateRange`) and CSV export. Populates the audit ledger with total records, +inflow units, -outflow units, and transactional valuation.
- **Reports Top Products & Top Customers Charts (`demoData.js`, `demoAdapter.js`)** — Aligned `DEMO_SALES_ANALYTICS.topProducts` and `topCustomers` schemas with Recharts vertical bar keys (`productName`, `quantitySold`, `customerName`, `invoiceCount`), restoring horizontal bars, Y-axis typography, and hover tooltips on `/reports`.
- **Demo Authentication Resilience & Preload Warning Elimination (`AuthContext.jsx`, `api.js`, `index.html`)** — Intercepted `POST /auth/login` for demo accounts in `api.js` and `demoAdapter.js`, and added graceful demo mode activation in `AuthContext.jsx`. Removed unconditional hero WebP preloads from global `index.html`, eliminating browser console warnings on non-landing routes.
- **Invoice Creation Crash Fix (`InvoiceCreatePage.jsx`, `demoData.js`)** — Resolved `Uncaught TypeError: Cannot read properties of undefined (reading 'charAt')` by guarding customer initial avatar computation with `(selectedCustomer.customerName || selectedCustomer.name || '?').charAt(0)` and normalizing all demo customer records with dual naming keys (`customerName` & `name`).
- **Dashboard Sales Area Graph Parity (`demoData.js`, `DashboardChartsSection.jsx`)** — Fixed the flat/zero sales line in the main dashboard chart. Provided dual `revenue` and `sales` properties on each daily data point in `DEMO_SALES_ANALYTICS.daily` and added defensive fallback `sales: d.revenue ?? d.sales ?? 0` in `DashboardChartsSection.jsx`.
- **Collections Table Crash Fix (`CollectionsPage.jsx`, `demoAdapter.js`)** — Resolved `Uncaught TypeError: data.payments is not iterable` by intercepting `/payments/collections` in `demoAdapter.js` with dataset summaries, method breakdowns (`byMethod`), and itemized receipts, while defensively guarding `visibleTotals` in `CollectionsPage.jsx` with `data?.payments || []`.
- **Credit Receivables & Ageing Breakdown Parity (`demoAdapter.js`, `demoData.js`)** — Rewired `/reports/outstanding` and `/reports/ageing` in `demoAdapter.js` to return matching schemas with summaries, credit limits, overdue day calculations, and bucketed invoices, restoring live data in the Outstanding and Ageing tabs on `/credits`.
- **Product Pricing & Catalog Inventory Parity (`demoData.js`, `demoAdapter.js`)** — Unified `DEMO_PRODUCTS` to supply both desktop table and mobile card property keys (`productName`, `hsnCode`, `newMRP`, `oldMRP`, `rate`, `gstPercentage`, `currentStockQty`, `effectiveStockQty`), eliminating zero rate and MRP displays across the product catalog and low-stock alerts.
- **Invoices & Purchases Table Parity (`demoData.js`, `demoAdapter.js`)** — Enriched demo invoices with `totals: { netTotal, grandTotal }` and customer objects, and populated demo purchases with comprehensive `supplierId` objects (`{ _id, id, name, gstin, phone }`), `totals`, and `COMPLETED` status, eliminating ₹0.00 figures and "Unknown Supplier" badges.
- **Team Management & Employee Detail Navigation (`EmployeesPage.jsx`, `demoData.js`, `demoAdapter.js`)** — Fixed inactive status, zero metrics, and broken `/employees/undefined` links by mapping `id: emp._id`, `isActive: true`, and populated productivity metrics (`invoicesCreatedCount`, `totalSalesGenerated`, `paymentsRecordedCount`, `lastActivityAt`, `sessionStats`), while guarding route navigation with `employee.id || employee._id`.
- **Team Analytics & Audit Logging Suites (`demoData.js`, `demoAdapter.js`)** — Added dedicated mock endpoints for `/analytics/employees`, `/analytics/employees/comparison`, `/analytics/sessions/summary`, and `/analytics/activity-log`, populating active sessions, leaderboard comparisons, and immutable audit logs.
- **Manual Entries & Khata Adjustments (`demoData.js`, `demoAdapter.js`)** — Added `DEMO_MANUAL_ENTRIES` and mock handlers for `/manual-entries` and `/manual-entries/customer/*`, displaying historical opening balance and khata adjustments.
- **Purchase & Inventory Intelligence Reports (`demoData.js`, `demoAdapter.js`)** — Wired complete reporting suites for `/reports/purchases/*` (`summary`, `supplier-wise`, `product-wise`, `status`, `inventory-flow`) and `/analytics/inventory/*` (`expiry-horizon`, `velocity`, `stock-risk`, `procurement`), populating batch expiry horizons and deterministic quartile sales velocity.

### 🧪 Client-Side Isolated Demo Mode & Hardcoded Credentials Pruning (`demoData.js`, `demoState.js`, `demoAdapter.js`, `LoginPage.jsx`, `Header.jsx`, `api.js`, `useStockSSE.js`, `AuthContext.jsx`, `SubscriptionContext.jsx`)
- **Demo Credentials Pruning (`LoginPage.jsx`)** — Completely removed the static demo credentials card (`Email: admin@bharat.com`, `Password: admin123`) from the authentication page, eliminating confusion and hardcoded account exposure.
- **1-Click Live Demo Mode (`LoginPage.jsx`, `demoState.js`)** — Added a prominent, enterprise-styled "Explore Live Demo Mode" button (`Sparkles` icon, "No Login Required" badge). Clicking immediately provisions an isolated client-side session without requiring email, password, or server authentication.
- **Zero-Server Network Isolation (`demoAdapter.js`, `api.js`, `useStockSSE.js`)** — Implemented an offline Axios mock adapter (`demoMockAdapter`) that intercepts 100% of outgoing requests client-side. Zero HTTP requests or cookies are sent over the wire, keeping live production/development databases completely untouched. Live stock Server-Sent Events (SSE) connections are safely bypassed in demo mode.
- **Realistic Indian Distribution Sample Data (`demoData.js`)** — Bundled a rich, realistic Indian wholesale pharmaceutical dataset featuring 12 medicines (Dolo 650mg, Augmentin 625 Duo, Pan 40, Azithral) with real HSNs, batch numbers, and expiry dates; 6 retail pharmacies with full khata ledgers; 6 itemized tax invoices; suppliers; purchase orders; collections; and 12-month analytics trends.
- **Sidebar Lock Icon Elimination & Route Unlocking (`SubscriptionContext.jsx`, `demoData.js`)** — Resolved the issue where all sidebar navigation items displayed upgrade lock icons in Demo Mode. Aligned demo subscription features with uppercase SaaS feature enums and added an authoritative `isDemoModeActive()` bypass in `canAccess` and `canAccessRoute`, unlocking the complete distribution suite for frictionless evaluation.
- **Executive Dashboard KPI Parity & Mock Wiring (`demoData.js`, `demoAdapter.js`)** — Fixed blank and zero metrics across Dashboard KPI summary cards. Corrected property keys in `DEMO_DASHBOARD_STATS` (`todayInvoices: 4`, `monthSales: 390546.00`, `totalProducts: 12`, `totalCustomers: 6`, `growth`), exported `DEMO_CREDIT_STATS`, and wired mock handlers for `GET /reports/credit-stats`, `GET /reports/recent-payments`, `GET /reports/outstanding`, and `GET /reports/ageing`. Cards now accurately render Today's Sales (₹42,850 • 4 bills), Monthly Revenue (₹3,90,546 • MTD Invoiced), Cash Collections (₹60,400 • 3 receipts), Receivables & Dues (₹1,48,220 • ₹38,400 Overdue), and Active Catalog (12 • 2 low stock).
- **Public SaaS Subscription Tiers & Duration Pricing (`demoData.js`, `demoAdapter.js`)** — Fixed `₹NaN/mo` calculation errors on `/subscription`. Defined canonical `DEMO_PLANS` reflecting the public tier structure: **Starter** (₹299/mo), **Business** (₹499/mo), and **Professional** (₹699/mo) with active tier attribution, dynamic duration discount selectors (1, 3, 6, 12 months), and itemized feature checklists.
- **Read-Only Conversion Guard (`demoAdapter.js`)** — Enforced a safe read-only guard: prospective clients can freely explore every page, filter, search, and view PDF invoices. Any mutation attempt (e.g. creating an invoice or recording a payment) returns a friendly conversion prompt directing them to create a free trial account.
- **Workspace Demo Indicator & Clean Exit (`Header.jsx`, `AuthContext.jsx`)** — Rendered a distinctive amber indicator pill in the workspace header (`🧪 Demo Mode • Offline Preview • [Exit]`). Exiting immediately flushes all demo tokens, SWR caches, and returns safely to the login screen.

### 🛡️ Auth Flow Stabilization & Frame-0 Zero-CLS Rendering (`LoginPage.jsx`, `RegisterPage.jsx`, `App.jsx`, `index.html`)
- **First-Visit Flick & Spring Pop-In Elimination (`LoginPage.jsx`, `RegisterPage.jsx`)** — Surgically removed delayed Framer Motion spring physics (`initial="hidden"`, `opacity: 0`, `delayChildren: 0.2`, `staggerChildren: 0.1`, and spring `y: 20` bounce). Auth cards and form inputs now mount synchronously on Frame 0 with rock-solid static positioning. Completely eliminates the ~200ms blank card delay, out-of-order static submit button appearance, compound nested layout jumps on "Business Details", and React StrictMode double-mount jitter in development.
- **Zero-Displacement In-Flow Navigation Header (`LoginPage.jsx`, `RegisterPage.jsx`)** — Replaced disconnected `absolute top-4 left-4 right-4` floating bar with a stable in-flow header positioned directly above the auth card (`w-full max-w-md mb-3 flex items-center justify-start`) with an `<ArrowLeft>` Back to Home link. Updated container wrapper from `overflow-hidden` to `overflow-y-auto py-8 sm:py-12`, guaranteeing the navigation button never collides with or overlaps the card title/logo on short viewports (< 750px) or mobile devices with onscreen keyboards. Removed redundant duplicate "← Back to Home" links at the card bottom.
- **Theme Canvas Initial Paint Invariance (`index.html`)** — Enhanced inline theme script so unconfigured first visits on auth routes (`isAuthRoute`) default to deep slate (`#020617` / `#09090b`), completely eliminating the high-contrast white-to-black flash on machines with Windows OS light theme defaults.
- **Synchronous Public Route Frame-0 Resolution (`App.jsx`)** — Added instant `hasToken` check in `PublicRoute`. Unauthenticated visitors hitting `/login` or `/register` render children synchronously on Frame 0 with zero ticks waiting for auth state, eliminating any transient `PageLoader` pulse flashes.

### 🎨 Public Theme Switcher Consolidation (`LandingNav.jsx`, `LoginPage.jsx`, `RegisterPage.jsx`, `PrivacyPolicyPage.jsx`, `TermsPage.jsx`)
- **Single Source of Truth for Public Themes** — Established Landing Page navigation (`LandingNav.jsx`) as the exclusive theme control for all unauthenticated visitors.
- **Floating Switcher Pruning** — Removed redundant `<ThemeToggle />` buttons from `/login`, `/register`, `/privacy-policy`, and `/terms`. Public pages now inherit and display the single cohesive theme selected on the landing page without contradictory floating controls. Post-login workspace theme toggle in `Header.jsx` and `Sidebar.jsx` remains active and untouched.

### ⚡ Hardware-Tiered Desktop Scroll Reveal Animations (`ScrollReveal.jsx`, `LandingPage.jsx`)
- **Enterprise Scroll Reveal Component (`ScrollReveal.jsx`)** — Created an enterprise-grade, GPU-accelerated scroll reveal wrapper animating strictly via `transform` (`y: 20px`) and `opacity` with smooth ease curves (`[0.16, 1, 0.3, 1]`) and one-shot triggers (`viewport={{ once: true, margin: '-60px' }}`).
- **Strict Mobile & Touch Bypass** — Wired directly to canonical `useDeviceType()` and `useMotionConfig()` hooks. When `isMobile`, `isTouchDevice`, or `prefersReducedMotion` is detected, Framer Motion is completely bypassed to render a vanilla HTML `<div>` with 0ms JavaScript animation overhead, ensuring uninterrupted 60fps/120fps touch scrolling and battery efficiency on mobile screens.
- **Hardware-Tiered Desktop Adaptation** — Automatically adapts to hardware capabilities via `usePerformanceMode()`: on low-spec desktop hardware (`isLowPerformance`), stagger delays are removed and fast durations (`configDuration.fast`) are applied automatically.
- **Core Web Vitals Guard (`LandingPage.jsx`, `HeroSection.jsx`)** — Wrapped below-the-fold showcase and feature sections in `<ScrollReveal>` while keeping `<HeroSection>` eager and un-delayed for instantaneous Largest Contentful Paint (LCP).

### 📐 Hero Spacing, Navbar Clearance & Showcase Metric Rescaling (`HeroSection.jsx`)
- **Fixed Navbar Clearance & Container Breathing Room (`HeroSection.jsx`)** — Expanded hero section top padding from cramped `pt-16` / `pt-20` to generous `pt-28 pb-12 sm:pt-32 sm:pb-16 lg:pt-36 lg:pb-20 xl:pt-40`, and added explicit container top padding (`pt-2 sm:pt-4 lg:pt-5`). Ensures ~50px–80px of clear visual cushion below the fixed navbar (`h-16`/`sm:h-20`), eliminating any collision or cramped appearance for the eyebrow and dashboard preview.
- **Proportional Showcase Metric & Snapshot Rescaling (`HeroSection.jsx`)** — Rescaled oversized floating notification cards from `w-[256px]` down to `w-[204px] 2xl:w-[214px]` (~22% width reduction). Reduced card padding from `p-3.5` to `p-2.5`, trimmed gap to `gap-2.5`, scaled down icon containers from 36px (`w-9 h-9`) to 28px (`w-7 h-7 rounded-lg`), and refined typography (`text-[9px]`, `text-[11px]`, `text-xs`). Tucked cards tightly against the dashboard bezel (`-right-3 xl:-right-4 2xl:-right-6`) so they act as sleek, proportional SaaS KPI accents rather than obscuring central dashboard charts.
- **Title Spacing Defense (`HeroSection.jsx`)** — Added an explicit space inside `<span className="lg:block">The Operating System for </span>` to guarantee clean separation between "for" and "Modern" regardless of CSS display or whitespace collapsing across browsers.
- **Reassurance Notice Breathing Room (`HeroSection.jsx`)** — Expanded top spacing above the dynamic trial/pricing microcopy (`mt-6 sm:mt-7 pt-1.5 text-xs text-slate-400`), providing comfortable, professional breathing room below the action CTA buttons.
...
### ⏰ Indian Standard Time (IST) Synchronization & Shared Export Engine Alignment (`stockMovementService.ts`, `excel.ts`, `formatters.js`, `InventoryLedgerPage.jsx`, `PricingSection.jsx`)
- **Root Cause of 5.5-Hour Excel Lag** — Excel date serial numbers lack timezone metadata. In the shared export engine (`excel.ts`), raw UTC `Date` objects were written directly to ExcelJS cells, causing Microsoft Excel and Google Sheets to evaluate timestamps in UTC (e.g. `06:15 AM` instead of `11:45 AM`), lagging 5 hours and 30 minutes behind the UI and PDF/CSV exports.
- **Global Excel Engine IST Alignment (`excel.ts`)** — Enhanced `buildWorkbook` to shift date and datetime cell values as well as summary `Generated At` by the Indian Standard Time delta (`+5.5 hours`), completely eliminating host timezone cancellation (`serverOffsetMs`). Because ExcelJS derives serial date numbers directly from UTC epoch timestamps, applying the `+5.5 hours` shift guarantees that Microsoft Excel and Google Sheets display the exact `Asia/Kolkata` time (e.g. `01:30 PM`, `10:51 AM`) matching the UI down to the exact minute across all platforms.
- **Midnight-to-5:30 AM Filter Cutoff Resolution (`stockMovementService.ts`)** — Resolved the discrepancy where querying by date (e.g. "Today") parsed strings as UTC midnight (`00:00:00Z`), cutting off movements created between 12:00 AM and 5:30 AM IST. Integrated `parseISTDateBoundary` to anchor search boundaries strictly between `00:00:00.000+05:30` and `23:59:59.999+05:30`.
...
- **Collections Page Filter Loading & Zero-CLS Skeletons (`CollectionsPage.jsx`, `useSWR.js`)** — Resolved the issue where clicking date presets (`Today`, `Yesterday`, `Last 7 Days`, `This Month`, `All Dates`, `Custom Range`) or any filter showed no loading state, leaving stale data frozen on screen until fresh data abruptly snapped in. Enhanced `useSWR` to support `keepPreviousData: false`, trigger `isLoading: true`, and clear stale data on key changes without cache. Equipped `CollectionsPage.jsx` with query provenance tracking (`_queryKey`), zero-CLS `ShimmerBone` metric states inside the 4 KPI cards (keeping cards, headers, and colored badge icons completely stable), channel allocation bar shimmer pulses, active `CollectionsTableSkeleton` table rendering during filter fetches, and prominent `<RefreshIndicator size="sm" showText />` header feedback.
...
- **Side-by-Side 2-Column Hero Layout (`HeroSection.jsx`)** — Re-architected desktop layout into a balanced 2-column grid (`grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-10 xl:gap-14 items-center`):
  - **Left Column (`lg:col-span-5`)**: Uppercase eyebrow (`TRUSTED BY INDIAN DISTRIBUTORS, WHOLESALERS & RETAIL ENTERPRISES`), bold 2-line title (`The Operating System for Modern Indian Distribution`), comprehensive commercial description, side-by-side CTA buttons (`Get Started ->` and `Explore Connected Workflow`), and dynamic trial microcopy.
  - **Right Column (`lg:col-span-7`)**: Embeds `ProductWindow` referencing newly versioned, cache-immune assets (`/landing/product/dashboard-v2.webp`) and **2 elevated floating telemetry badges** (`hidden xl:flex`) offset to the outer perimeter (`-right-6 2xl:-right-10`) with distinct `rounded-2xl` glassmorphism, multi-layer drop shadows (`shadow-[0_20px_50px_rgba(0,0,0,0.5)]`), and ambient border highlights:
    - *Monthly Revenue*: Header chip `Live Telemetry • Active`, blue chart icon, `₹3,90,546 (+93.4%)`, `vs. last month`.
    - *Catalog*: Header chip `Catalog Status • Synchronized`, neutral package icon, `{productsCount} SKUs • {batchesCount} Batches`, `Real-time inventory`.
    - *Visual Contrast & Separation*: The distinct elevation, outer offset, and header badges unambiguously communicate that these are external real-time feed overlays rather than elements of the dashboard screenshot.
    - *Payment Received Card Elimination*: Removed the redundant payment card that previously overlapped the top-right KPI metrics, achieving clean visual separation and breathing room.
- **Hero Showcase Cache-Immune Asset Versioning (`dashboard-v2*`, `ProductWindow.jsx`, `index.html`)** — Created versioned public assets (`dashboard-v2.webp`, `dashboard-v2-mobile.webp`, `dashboard-v2-sm.webp`, `dashboard-v2.png`) and updated `ProductWindow.jsx` with robust URL query parameter splitting. Completely eliminates stale client browser cache lock-in and guarantees that all visitors instantly receive the newly captured dashboard without CSS overflow.
- **7-Column Capability Strip Redesign (`CapabilityStrip.jsx`)** — Transformed capability section into a structured 7-column grid (`grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4 lg:gap-3 xl:gap-5`) with uppercase header `ENGINEERED FOR INDIAN DISTRIBUTION`, blue outline icons, bold titles, and parenthetical subtitles (`GST Invoicing / (HSN & Tax Splits)`, `Batch-Level / Inventory & Expiry`, `B2B Customer / Khata & Ledgers`, `Dual-Copy Invoicing / (Customer & Transport)`, `Supplier & / Purchase Orders`, `Financial Telemetry / & Cash Flow`, `Role-Based Access / (Admin & Staff)`).
- **Billing & Tax Showcase Polish (`BillingShowcase.jsx`, `ProductWindow.jsx`)** — Added uppercase category eyebrow `BILLING & TAX`, rounded blue-tinted icon containers (`w-10 h-10 rounded-xl bg-blue-500/10 border border-blue-500/20`), and updated `ProductWindow` to support `variant="window"` for clean macOS chrome without ambient glow.
- **Showcase Screenshot Re-Capture with Zero Overflow (`dashboard.png`, `dashboard.webp`, `dashboard-v2.webp`)** — Re-captured all 6 showcase screenshots at 1440×900 Retina 2x via Puppeteer following the 5-grid card overflow resolution in `DashboardKPIGrid.jsx`. Verified that in the newly generated public assets (`dashboard-v2.webp`, `dashboard-v2-mobile.webp`, `dashboard-v2-sm.webp`), the KPI cards render in 3 comfortable columns with plenty of breathing room, zero horizontal overflow, and flawless alignment.
...
### 🌓 Public Theme Continuity & Legal Pages Light Mode Polish (`ThemeContext.jsx`, `index.html`, `index.css`)
- **Public Route Theme Continuity** — Fixed an issue where switching to light mode on the landing page caused `/login` and `/register` to revert to dark mode. Expanded public route scope in `ThemeContext.jsx` and `index.html` to cover `/login`, `/register`, `/privacy-policy`, and `/terms`, synchronizing user preferences across all public entry points without dark mode flash.
...
### 🚀 Lighthouse Performance, Zero-CLS, Best Practices (100) & Agentic Web (`ProductWindow.jsx`, `AuthContext.jsx`, `llms.txt`, `index.html`)
- **Third-Party Cookie & Cross-Origin Latency Elimination (`AuthContext.jsx`, `SubscriptionContext.jsx`, `subscriptionService.js`)** — Deferred `checkAuth()`, `heartbeat()`, and `getSubscription()` on public marketing routes (`/landing`, `/privacy-policy`, `/terms`). Passed `{ withCredentials: false }` for public `/saas/plans` queries, completely eliminating the cross-origin `onrender.com token` cookie warning and cutting **2.8s** of critical path network latency. Elevates Lighthouse Best Practices audit to **100**.
- **Official AI Agentic Web Accessibility (`frontend/public/llms.txt`)** — Implemented an official [llmstxt.org](https://llmstxt.org) standard Markdown file featuring the required H1 title header, blockquote summary, and structured links to all core enterprise features and legal policies. Elevates Lighthouse Agentic Browsing score to **3/3**.
- **Three-Tier Responsive WebP Architecture (`convertWebp.mjs`, `ProductWindow.jsx`)** — Added an ultra-compact small-mobile tier (540px width, quality 75) reducing single view weight to **~9.5–12.9 KB (a 97% reduction from 383 KB PNG)**. Updated `ProductWindow.jsx` with `<source media="(max-width: 640px)" type="image/webp" srcSet={smWebp} />`.
- **Zero-CLS Responsive `<picture>` Architecture (`ProductWindow.jsx`, `HeroSection.jsx`)** — Set explicit `width="2880" height="1800"` on `aspect-[16/10]` containers and anchored the ambient lighting glow to `top-48`, permanently eliminating the **0.200 mobile CLS** down to **0.000**.
- **High-Priority Hero WebP Preload (`index.html`)** — Added media-queried preloads for `dashboard-sm.webp` (<=640px), `dashboard-mobile.webp` (641px–1024px), and `dashboard.webp` (>1024px) with `fetchpriority="high"`, dropping LCP load delay to 40ms.
...
- **Dynamic Exact Canonical URL (`index.html`)** — Replaced static root canonical tag with `#canonical-tag` script computing `window.location.origin + window.location.pathname`, resolving Lighthouse SEO audit warning for `/landing` and subpages (achieving 100 SEO).
- **Origin Isolation & COOP Security Header (`vercel.json`)** — Added `Cross-Origin-Opener-Policy: same-origin-allow-popups` to global security headers, elevating window isolation while maintaining Razorpay checkout support. Added 1-year immutable caching for `/landing/` static assets.
